...
public class DatabaseSupportImpl implements Database {
    ...
    public DatabaseSupportImpl(PersistenceManager persistenceManager) {
        ...
        switchDevicesRepository = repositoryFactory.getSwitchConnectedDeviceRepository();
    }
        ...
        return transactionManager.doInTransaction(() -> {
            ...
            GraphTraversal<?, ?> rawTraversal = framedGraph.traverse(input -> input.withSack(0)
                    ...
                    .repeat(__.outE(IslFrame.FRAME_LABEL).has(IslFrame.STATUS_PROPERTY, "active")
                            .sack(sum).by("cost")
                            .inV().simplePath())
                    .until(__.hasLabel(SwitchFrame.FRAME_LABEL).has(SwitchFrame.SWITCH_ID_PROPERTY, dst.toString())
    ...
    public Collection<TransitVlan> getTransitVlans(PathId forwardPathId, PathId reversePathId) {
        return transactionManager.doInTransaction(() ->
                transitVlanRepository.findByPathId(forwardPathId, reversePathId));
    }
