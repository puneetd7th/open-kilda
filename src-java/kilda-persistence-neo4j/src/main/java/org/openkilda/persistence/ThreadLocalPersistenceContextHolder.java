...
public final class ThreadLocalPersistenceContextHolder implements PersistenceContextManager {
    ...
    public void initContext() {
        ...
        log.debug("Initializing persistence context");
        initFlag.set(true);
    }
    ...
    public void closeContext() {
        if (isContextInitialized()) {
            log.debug("Closing the persistence context");
            initFlag.remove();
        }
        ...
        if (currentGraph != null) {
            try {
                log.debug("Closing the framed graph: {}", currentGraph);
                currentGraph.close();
            } catch (IOException e) {
    ...
    public void setCurrentGraph(DelegatingFramedGraph graph) {
        log.debug("Set the framed graph as current: {}", graph);
        graphs.set(graph);
    }
    ...
    public void removeCurrentGraph() {
        log.debug("Remove(un-bound) the current framed graph");
        graphs.remove();
    }
}
