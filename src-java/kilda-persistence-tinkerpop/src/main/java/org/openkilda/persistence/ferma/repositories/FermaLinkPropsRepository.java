 ...
 */
class FermaLinkPropsRepository extends FermaGenericRepository<LinkProps, LinkPropsData>
    ...
    public Collection<LinkProps> findAll() {
        return framedGraph().traverse(g -> g.V().hasLabel(LinkPropsFrame.FRAME_LABEL))
    ...
    public Collection<LinkProps> findByEndpoints(SwitchId srcSwitch, Integer srcPort,
        ...
        return framedGraph().traverse(g -> {
            GraphTraversal<Vertex, Vertex> traversal = g.V().hasLabel(LinkPropsFrame.FRAME_LABEL);
            if (srcSwitch != null) {
                traversal = traversal.has(LinkPropsFrame.SRC_SWITCH_PROPERTY,
                        SwitchIdConverter.INSTANCE.map(srcSwitch));
            }
            if (srcPort != null) {
            ...
            if (dstSwitch != null) {
                traversal = traversal.has(LinkPropsFrame.DST_SWITCH_PROPERTY,
                        SwitchIdConverter.INSTANCE.map(dstSwitch));
            }
            if (dstPort != null) {
    ...
    @Override
    public LinkProps add(LinkProps entity) {
        LinkPropsData data = entity.getData();
        if (data instanceof LinkPropsFrame) {
            throw new IllegalArgumentException("Can't add entity " + entity + " which is already framed graph element");
        }
        return transactionManager.doInTransaction(() -> new LinkProps(LinkPropsFrame.create(framedGraph(), data)));
    }
}
