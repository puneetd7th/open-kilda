...
public abstract class KildaBaseEdgeFrame extends AbstractEdgeFrame {
    ...
    public void setProperty(String name, Object value) {
        if (!name.equals(KildaBaseVertexFrame.TIME_MODIFY_PROPERTY)) {
            setProperty(KildaBaseVertexFrame.TIME_MODIFY_PROPERTY, InstantStringConverter.INSTANCE.map(Instant.now()));
        }
        super.setProperty(name, value);
    }

    public Instant getTimeCreate() {
        return InstantStringConverter.INSTANCE.map((String) getProperty(KildaBaseVertexFrame.TIME_CREATE_PROPERTY));
    }
    ...
    public Instant getTimeModify() {
        return InstantStringConverter.INSTANCE.map((String) getProperty(KildaBaseVertexFrame.TIME_MODIFY_PROPERTY));
    }
    ...
    public static <U extends KildaBaseEdgeFrame> U addNewFramedEdge(FramedGraph graph, VertexFrame source,
        ...
        String now = InstantStringConverter.INSTANCE.map(Instant.now());
