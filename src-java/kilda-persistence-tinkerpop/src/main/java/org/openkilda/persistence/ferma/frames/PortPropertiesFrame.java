...
public abstract class PortPropertiesFrame extends KildaBaseVertexFrame implements PortPropertiesData {
    ...
    public SwitchId getSwitchId() {
        return traverse(v -> v.in(OWNS_SWITCH_EDGE).hasLabel(SwitchFrame.FRAME_LABEL)
    ...
    public Switch getSwitchObj() {
        return Optional.ofNullable(traverse(v -> v.in(OWNS_SWITCH_EDGE)
    ...
    public void setSwitchObj(Switch switchObj) {
        getElement().edges(Direction.IN, OWNS_SWITCH_EDGE).forEachRemaining(Element::remove);
    ...
    public static PortPropertiesFrame create(FramedGraph framedGraph, PortPropertiesData data) {
        PortPropertiesFrame frame = KildaBaseVertexFrame.addNewFramedVertex(framedGraph, FRAME_LABEL,
                PortPropertiesFrame.class);
