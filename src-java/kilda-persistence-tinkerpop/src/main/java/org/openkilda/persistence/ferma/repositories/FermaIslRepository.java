 ...
 */
public class FermaIslRepository extends FermaGenericRepository<Isl, IslData> implements IslRepository {
    ...
    public Collection<Isl> findAll() {
        return framedGraph().traverse(g -> g.E().hasLabel(IslFrame.FRAME_LABEL))
                .toListExplicit(IslFrame.class).stream()
                .map(Isl::new).map(this::addIslConfigToIsl).collect(Collectors.toList());
    }
    ...
    public Optional<Isl> findByEndpoints(SwitchId srcSwitchId, int srcPort, SwitchId dstSwitchId, int dstPort) {
        return findIsl(srcSwitchId, srcPort, dstSwitchId, dstPort)
                .map(Isl::new).map(this::addIslConfigToIsl);
    }
    ...
    public Collection<Isl> findActiveAndOccupiedByFlowPathWithAvailableBandwidth(
        ...
        FlowPathFrame flowPath = framedGraph().traverse(g -> g.V().hasLabel(FlowPathFrame.FRAME_LABEL)
        ...
        String activeSwitchStatus = SwitchStatusConverter.INSTANCE.map(SwitchStatus.ACTIVE);
        String flowEncapType = FlowEncapsulationTypeConverter.INSTANCE.map(flowEncapsulationType);
        return framedGraph().traverse(g -> g.V().hasLabel(SwitchFrame.FRAME_LABEL)
                .has(SwitchFrame.STATUS_PROPERTY, activeSwitchStatus)
                .where(__.out(SwitchPropertiesFrame.HAS_BY_EDGE).hasLabel(SwitchPropertiesFrame.FRAME_LABEL)
                        .has(SwitchPropertiesFrame.SUPPORTED_TRANSIT_ENCAPSULATION_PROPERTY, flowEncapType))
                .as("source")
                .outE(IslFrame.FRAME_LABEL)
                .has(IslFrame.STATUS_PROPERTY, IslStatusConverter.INSTANCE.map(IslStatus.ACTIVE))
                ...
                .where(__.out(SwitchPropertiesFrame.HAS_BY_EDGE).hasLabel(SwitchPropertiesFrame.FRAME_LABEL)
                        .has(SwitchPropertiesFrame.SUPPORTED_TRANSIT_ENCAPSULATION_PROPERTY, flowEncapType))
                .as("destination")
                .in(PathSegmentFrame.DESTINATION_EDGE).hasLabel(PathSegmentFrame.FRAME_LABEL)
    ...
    public Collection<Isl> findAllActiveByEncapsulationType(FlowEncapsulationType flowEncapsulationType) {
        ...
        return framedGraph().traverse(g -> g.E().hasLabel(IslFrame.FRAME_LABEL)
                .has(IslFrame.STATUS_PROPERTY, IslStatusConverter.INSTANCE.map(IslStatus.ACTIVE))
                .as("forward")
                .outV().hasLabel(SwitchFrame.FRAME_LABEL)
                .has(SwitchFrame.STATUS_PROPERTY, activeSwitchStatus)
                .where(__.out(SwitchPropertiesFrame.HAS_BY_EDGE).hasLabel(SwitchPropertiesFrame.FRAME_LABEL)
                        .has(SwitchPropertiesFrame.SUPPORTED_TRANSIT_ENCAPSULATION_PROPERTY, flowEncapType))
                .select("forward")
    ...
    public Collection<Isl> findSymmetricActiveWithAvailableBandwidth(long requiredBandwidth,
                                                                     FlowEncapsulationType flowEncapsulationType) {
        String activeSwitchStatus = SwitchStatusConverter.INSTANCE.map(SwitchStatus.ACTIVE);
        String activeIslStatus = IslStatusConverter.INSTANCE.map(IslStatus.ACTIVE);
        String flowEncapType = FlowEncapsulationTypeConverter.INSTANCE.map(flowEncapsulationType);
        return framedGraph().traverse(g -> g.E().hasLabel(IslFrame.FRAME_LABEL)
    ...
    @Override
    public Isl add(Isl entity) {
        IslData data = entity.getData();
        ...
        IslData framedData = transactionManager.doInTransaction(() -> {
        ...
        return addIslConfigToIsl(new Isl(framedData));
    }
}
