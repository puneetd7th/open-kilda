...
public abstract class FlowHistoryFrame extends KildaBaseVertexFrame implements FlowHistoryData {
    ...
    public Instant getTimestamp() {
        return InstantStringConverter.INSTANCE.map((String) getProperty("timestamp"));
    }

    @Override
    public void setTimestamp(Instant timestamp) {
        setProperty("timestamp", InstantStringConverter.INSTANCE.map(timestamp));
    }
    ...
    public void setFlowEvent(FlowEvent flowEvent) {
        getElement().edges(Direction.IN, HISTORY_LOG_EDGE).forEachRemaining(Element::remove);
    ...
    public static FlowHistoryFrame create(FramedGraph framedGraph, FlowHistoryData data) {
        FlowHistoryFrame frame = KildaBaseVertexFrame.addNewFramedVertex(framedGraph, FRAME_LABEL,
                FlowHistoryFrame.class);
