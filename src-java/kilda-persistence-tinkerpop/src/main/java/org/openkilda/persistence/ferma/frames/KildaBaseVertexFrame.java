...
public abstract class KildaBaseVertexFrame extends AbstractVertexFrame {
    ...
    public void setProperty(String name, Object value) {
        if (!name.equals(TIME_CREATE_PROPERTY) && !name.equals(TIME_MODIFY_PROPERTY)) {
            setProperty(TIME_MODIFY_PROPERTY, InstantStringConverter.INSTANCE.map(Instant.now()));
        }
        super.setProperty(name, value);
    }

    public Instant getTimeCreate() {
        return InstantStringConverter.INSTANCE.map((String) getProperty(TIME_CREATE_PROPERTY));
    }
    ...
    public Instant getTimeModify() {
        return InstantStringConverter.INSTANCE.map((String) getProperty(TIME_MODIFY_PROPERTY));
    }
